<template>
  <v-layout>
    ...
    <v-app-bar color="#16496E" density="compact" elevation="0" class="app-header">
      ...
      <v-btn prepend-icon="mdi-logout" variant="text" color="white" @click="handleLogout">
        Sair
      </v-btn>
    </v-app-bar>
...
const unwatchMobile = computed(() => {
  adjustLayoutForScreenSize()
  return mobile.value
})

const menuItems = [
  { title: 'Home', icon: 'mdi-home', value: 'home', to: '/admin/dashboard' },

  { title: 'Postos', icon: 'mdi-gas-station', value: 'station', to: { name: 'StationAdmin' } },
  { title: 'Mapa', icon: 'mdi-map-marker', value: 'mapa', to: { name: 'MapAdmin' } },
  { title: 'Veículos', icon: 'mdi-car', value: 'veiculos', to: { name: 'VehicleAdmin' } },
  {
    title: 'Ocorrências',
  ...
  { title: 'Usuários', icon: 'mdi-account-group', value: 'usuarios', to: { name: 'UsersAdmin' } },
]

// Nome da seção atual, exibido no header. Sai do próprio menu, sem texto novo.
const currentSection = computed(() => {
  const active = menuItems.find((item) =>
    typeof item.to === 'string' ? item.to === route.path : item.to.name === route.name,
  )
